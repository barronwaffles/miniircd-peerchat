...
class Channel(object):
    #CHANKEY_lobby=":\\"
    #CHANKEY_lobby=":\\"
    ...
    def __send_names(self, arguments, for_join=False):
        ...
        for (i, channelname) in enumerate(channelnames):
            ...
            if for_join:
                ...
                #self.message(":SERVER!SERVER@* NOTICE :" + self.nickname + " has joined "+channel.name)
    ...
    def __command_handler(self, command, arguments):
        ...
        def join_handler():
            ...
            if arguments[0] == "0":
                ...
                self.channels = {}
                return
            #self.message(":SERVER!SERVER@* NOTICE :" + self.nickname + " has joined "+arguments[0])
        ...
        def mode_handler():
            ...
            if server.has_channel(targetname):
                ...
                if flag == "+k":
                    ...
                    print("lolnope")
                    self.reply("MODE "+arguments[0]+" "+arguments[1]+" "+arguments[2])
        ...
        try:
            ...
            print("421 %s %s :Unknown command" % (self.nickname, command))
    ...
    def message_channel(self, channel, command, message, include_self=False):
        ...
        for client in channel.members:
            if client != self or include_self:
                client.message(line)
    def message_channel_as(self, user, channel, command, message, include_self=False):
        line = ":%s %s %s" % (user.prefix, command, message)
        for client in channel.members:
            if client != self or include_self:
                client.message(line)
